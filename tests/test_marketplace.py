...
def _clean_katalog(monkeypatch, tmp_path):
    ...
    kat_path.write_text(json.dumps(katalog_verileri, ensure_ascii=False, indent=2), encoding="utf-8")
...
class TestYukle:
    ...
    def test_yukle_kaynaksiz_plugin(self, monkeypatch, tmp_path):
        import reymen.sistem.marketplace as m
        kat = [
            {"ad": "kaynaksiz", "versiyon": "1.0", "aciklama": "no source",
             "kaynak": "", "yazar": "test", "lisans": "MIT",
             "etiketler": [], "bagimliliklar": [], "dokuman": ""}
        ]
...
class TestVarsayilanYaml:
    def test_varsayilan_yaml_olustur(self, tmp_path):
        ...
        dizin.mkdir()
        p = {"ad": "test_plugin", "versiyon": "2.0.0", "aciklama": "test desc",
             "yazar": "author", "lisans": "MIT"}
