...
def _mock_http_error(code: int = 400, body: str = "error") -> urllib.error.HTTPError:
    ...
    return urllib.error.HTTPError(
        url="http://example.com", code=code, msg="Error",
...
class TestGoogleOAuthProvider:
    ...
    def test_callback_handler_http_error(self):
        ...
        with patch.dict(os.environ, {
            ...
            error_resp = urllib.error.HTTPError(
                ...
                hdrs={}, fp=io.BytesIO(b"invalid_grant")
            )
        ...
        with patch.dict(os.environ, {
            ...
            mock_responses = [
                ...
                urllib.error.HTTPError(
                    ...
                    hdrs={}, fp=io.BytesIO(b"invalid_token")
                ),
            ]
            with patch("urllib.request.urlopen", side_effect=mock_responses):
    ...
    def test_token_refresh_basarili(self):
        ...
        with patch.dict(os.environ, {
            ...
            with patch("urllib.request.urlopen", return_value=_mock_response(refresh_data)):
    ...
    def test_get_user_info_http_error(self):
        ...
        with patch.dict(os.environ, {
            ...
            error_resp = urllib.error.HTTPError(
                ...
                hdrs={}, fp=io.BytesIO(b"access_denied")
            )
            with patch("urllib.request.urlopen", side_effect=error_resp):
...
class TestGitHubOAuthProvider:
    ...
    def test_callback_handler_basarili(self):
        ...
        with patch.dict(os.environ, {
            ...
            p = GitHubOAuthProvider()
            token_data = {"access_token": "gho_token123", "token_type": "bearer", "scope": "repo"}
            user_data = {"id": 12345, "email": "dev@github.com", "name": "Dev", "login": "devuser", "avatar_url": "https://avatars.github.com/u/12345"}
            mock_responses = [
                _mock_response(token_data),
    ...
    def test_callback_handler_http_error(self):
        ...
        with patch.dict(os.environ, {
            ...
            p = GitHubOAuthProvider()
            error_resp = urllib.error.HTTPError(
                url="http://example.com", code=401, msg="Bad creds",
                hdrs={}, fp=io.BytesIO(b"bad_verification_code")
            )
            with patch("urllib.request.urlopen", side_effect=error_resp):
                with pytest.raises(OAuthError):
                    p.callback_handler("bad-code")
    ...
    def test_token_refresh_hata_firlatir(self):
        ...
        with patch.dict(os.environ, {
            ...
            assert "sureklidir" in str(exc.value).lower() or "süreklidir" in str(exc.value)
    ...
    def test_get_user_info_http_error(self):
        ...
        with patch.dict(os.environ, {
            ...
            p = GitHubOAuthProvider()
            error_resp = urllib.error.HTTPError(
                url="http://example.com", code=403, msg="Rate limit",
                hdrs={}, fp=io.BytesIO(b"rate_limit_exceeded")
            )
            with patch("urllib.request.urlopen", side_effect=error_resp):
                with pytest.raises(OAuthError):
                    p.get_user_info("bad-token")
...
class TestDiscordOAuthProvider:
    ...
    def test_callback_handler_http_error(self):
        ...
        with patch.dict(os.environ, {
            ...
            p = DiscordOAuthProvider()
            error_resp = urllib.error.HTTPError(
                url="http://example.com", code=400, msg="Bad",
                hdrs={}, fp=io.BytesIO(b"invalid_code")
            )
            with patch("urllib.request.urlopen", side_effect=error_resp):
                with pytest.raises(OAuthError):
                    p.callback_handler("bad-code")
    ...
    def test_token_refresh_http_error(self):
        ...
        with patch.dict(os.environ, {
            ...
            error_resp = urllib.error.HTTPError(
                ...
                hdrs={}, fp=io.BytesIO(b"too_many_requests")
            )
            with patch("urllib.request.urlopen", side_effect=error_resp):
                with pytest.raises(OAuthError):
    ...
    def test_get_user_info_basarili(self):
        ...
        with patch.dict(os.environ, {
            ...
            p = DiscordOAuthProvider()
            user_data = {"id": "98765", "username": "discord_user", "global_name": "DiscordUser"}
            with patch("urllib.request.urlopen", return_value=_mock_response(user_data)):
                result = p.get_user_info("token")
