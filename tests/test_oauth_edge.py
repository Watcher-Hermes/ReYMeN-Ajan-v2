...
class TestOAuthEdgeCases:
    ...
    def test_discord_avatar_yok(self):
        ...
        with patch.dict("os.environ", {"DISCORD_CLIENT_ID": "d", "DISCORD_CLIENT_SECRET": "s"}, clear=True):
            p = DiscordOAuthProvider()
            m1 = MagicMock()
            m1.__enter__.return_value = m1
            m1.read.return_value = json.dumps({"access_token": "t", "expires_in": 3600}).encode()
            m2 = MagicMock()
            m2.__enter__.return_value = m2
            m2.read.return_value = json.dumps({"id": "123", "username": "u", "avatar": None}).encode()
    ...
    def test_github_userinfo_hatasi(self):
        ...
        with patch.dict("os.environ", {"GITHUB_CLIENT_ID": "g", "GITHUB_CLIENT_SECRET": "s"}, clear=True):
            p = GitHubOAuthProvider()
            m1 = MagicMock()
            m1.__enter__.return_value = m1
            m1.read.return_value = json.dumps({"access_token": "t", "token_type": "bearer"}).encode()
            err_fp = MagicMock()
