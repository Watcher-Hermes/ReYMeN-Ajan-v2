...
class TestGoogleOAuthMock:
    ...
    def test_callback_basarili(self):
        ...
        with patch.dict(os.environ, {"GOOGLE_CLIENT_ID": "id", "GOOGLE_CLIENT_SECRET": "secret"}):
            ...
            with patch.object(p, "_http_post", return_value={
                "access_token": "ya29.mock", "expires_in": 3600,
                "refresh_token": "rf", "token_type": "Bearer"
            }):
                with patch.object(p, "get_user_info", return_value={"sub": "123", "email": "a@b.com"}):
    ...
    def test_token_refresh(self):
        ...
        with patch.dict(os.environ, {"GOOGLE_CLIENT_ID": "id", "GOOGLE_CLIENT_SECRET": "secret"}):
            p = GoogleOAuthProvider()
            with patch.object(p, "_http_post", return_value={"access_token": "new_token", "expires_in": 3600}):
    ...
    def test_user_info(self):
        ...
        with patch.dict(os.environ, {"GOOGLE_CLIENT_ID": "id", "GOOGLE_CLIENT_SECRET": "secret"}):
            p = GoogleOAuthProvider()
            with patch.object(p, "_http_get", return_value={"sub": "12345", "email": "test@gmail.com"}):
    ...
    def test_token_expired(self):
        ...
        import time
        t = OAuthToken(access_token="old", provider="google", expires_in=1, obtained_at=time.time() - 10)
...
class TestGitHubOAuthMock:
    def test_login_url(self):
        ...
        with patch.dict(os.environ, {"GITHUB_CLIENT_ID": "gh_id", "GITHUB_CLIENT_SECRET": "gh_secret"}):
    ...
    def test_callback_basarili(self):
        ...
        with patch.dict(os.environ, {"GITHUB_CLIENT_ID": "id", "GITHUB_CLIENT_SECRET": "secret"}):
            p = GitHubOAuthProvider()
            # urllib.request.urlopen mock
            mock_resp = MagicMock()
            mock_resp.read.return_value = json.dumps({"access_token": "gh_token"}).encode()
            mock_resp.__enter__.return_value = mock_resp
            with patch("urllib.request.urlopen", return_value=mock_resp):
                with patch.object(p, "get_user_info", return_value={"id": 1, "login": "user"}):
    ...
    def test_callback_http_error(self):
        ...
        with patch.dict(os.environ, {"GITHUB_CLIENT_ID": "id", "GITHUB_CLIENT_SECRET": "secret"}):
            p = GitHubOAuthProvider()
            import urllib.error
            error_resp = urllib.error.HTTPError("http://example.com", 401, "Unauthorized", {}, None)
    ...
    def test_user_info(self):
        ...
        with patch.dict(os.environ, {"GITHUB_CLIENT_ID": "id", "GITHUB_CLIENT_SECRET": "secret"}):
            p = GitHubOAuthProvider()
            mock_resp = MagicMock()
            mock_resp.read.return_value = json.dumps({"login": "testuser", "id": 123}).encode()
            mock_resp.__enter__.return_value = mock_resp
            with patch("urllib.request.urlopen", return_value=mock_resp):
...
class TestDiscordOAuthMock:
    def test_login_url(self):
        ...
        with patch.dict(os.environ, {"DISCORD_CLIENT_ID": "dc_id", "DISCORD_CLIENT_SECRET": "dc_secret"}):
    ...
    def test_callback_basarili(self):
        ...
        with patch.dict(os.environ, {"DISCORD_CLIENT_ID": "id", "DISCORD_CLIENT_SECRET": "secret"}):
            ...
            mock_resp.read.return_value = json.dumps({
                "access_token": "dc_token", "expires_in": 3600,
                "refresh_token": "dc_refresh", "token_type": "Bearer"
            }).encode()
            mock_resp.__enter__.return_value = mock_resp
            with patch("urllib.request.urlopen", return_value=mock_resp):
                with patch.object(p, "get_user_info", return_value={"id": "1", "username": "user"}):
    ...
    def test_token_refresh(self):
        ...
        with patch.dict(os.environ, {"DISCORD_CLIENT_ID": "id", "DISCORD_CLIENT_SECRET": "secret"}):
            p = DiscordOAuthProvider()
            mock_resp = MagicMock()
            mock_resp.read.return_value = json.dumps({"access_token": "new_dc"}).encode()
            mock_resp.__enter__.return_value = mock_resp
            with patch("urllib.request.urlopen", return_value=mock_resp):
                token = p.token_refresh("old_refresh")
    ...
    def test_user_info(self):
        ...
        with patch.dict(os.environ, {"DISCORD_CLIENT_ID": "id", "DISCORD_CLIENT_SECRET": "secret"}):
            p = DiscordOAuthProvider()
            mock_resp = MagicMock()
            mock_resp.read.return_value = json.dumps({"id": "12345", "username": "testuser", "global_name": "Test"}).encode()
            mock_resp.__enter__.return_value = mock_resp
            with patch("urllib.request.urlopen", return_value=mock_resp):
                info = p.get_user_info("valid_token")
    ...
    def test_callback_http_error(self):
        ...
        with patch.dict(os.environ, {"DISCORD_CLIENT_ID": "id", "DISCORD_CLIENT_SECRET": "secret"}):
            p = DiscordOAuthProvider()
            import urllib.error
            error_resp = urllib.error.HTTPError("http://example.com", 400, "Bad Request", {}, None)
            with patch("urllib.request.urlopen", side_effect=error_resp):
                with pytest.raises(OAuthError):
                    p.callback_handler("bad_code")