...
alınırsa otomatik onarım adımları uygulanır ve sonuç kaydedilir. Hedef listeleme
  ve gönderme için `ReYMeN send --list` + `ReYMeN send --to <target>` kullanılır.
  '
  ...
  Hedef listeleme ve gönderme için `ReYMeN send --list` + `ReYMeN send --to <target>` kullanılır.
 |
...
| ReYMeN CLI Invocation (PYTHONPATH fix) | `references/ReYMeN-cli-invocation.md` |
...
| Cron Teslimat Davranışı (cron job'da `ReYMeN send` atlanır) | `references/cron-teslimat-davranisi.md` |
