---
name: software-development_reymen-ReYMeN-sync_references_2026-06-30-merge-session
description: 2026-06-30 Merge Session Log
title: "Software Development Reymen ReYMeN Sync References 2026 06 30 Merge Session"
version: 1.0.0
---
...
## Context
Complete 3-step merge of ReYMeN → Reymen. ReYMeN was updated, Reymen needed to absorb changes while preserving customizations.
...
ReYMeN `agent/` modules that Reymen root didn't have:
...
| File | ReYMeN | Reymen Old | Action |
|------|--------|-----------|--------|
| model_tools.py | 1,231L (tool defs) | 154L (benchmark) | Rename → benchmark_tools.py, copy ReYMeN |
| trajectory_compressor.py | 1,579L (training data) | 62L (runtime) | Rename → reymen_trajectory_compressor.py, copy ReYMeN |
| batch_runner.py | 1,321L (dataset batch) | 231L (parallel tasks) | Rename → reymen_batch_runner.py, copy ReYMeN |
| mcp_serve.py | 897L (message bridge) | 309L (agent expose) | Rename → reymen_mcp_serve.py, copy ReYMeN |
...
75 files exist in BOTH root/ and agent/ with different sizes. Root = Reymen simplified, agent/ = ReYMeN full. INTENTIONAL.

## GitHub Setup
- Repo: Watcher-ReYMeN/ReYMeN-Ajan (public)
- Self-update script: .hermes_sync.sh rewritten to pull from GitHub (not ReYMeN)
