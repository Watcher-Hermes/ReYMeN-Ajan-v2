...
> **Docker HOME pitfall:** In the official ReYMeN Docker layout, `/opt/data` is `HERMES_HOME`, but ReYMeN tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for ReYMeN-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
