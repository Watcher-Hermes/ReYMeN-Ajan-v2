...
title: "Software Development ReYMeN S6 Container Supervision References Key Files"
version: 1.0.0
---
...
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-ReYMeN-setup. |
...
| `docker/main-wrapper.sh` | The container's CMD. Routes user args, drops to ReYMeN via `s6-setuidgid`, exec's the chosen program. |
| `docker/s6-rc.d/main-ReYMeN/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main ReYMeN runs as the CMD, not as a supervised service. |
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `reymen gateway start/stop/restart` and routes to s6 when running in a container. |
