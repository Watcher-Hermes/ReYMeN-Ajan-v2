---
name: productivity_teams-meeting-pipeline
...
|:-----|:------|
| **Kim?** | Tüm ajanlar |
| **Ne?** | Operate the Teams meeting summary pipeline via ReYMeN CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions. |
| **Nerede?** | productivity/ |
...
Everything operator-facing is a `ReYMeN teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
```bash
ReYMeN teams-pipeline validate              # config snapshot — run first after any change
ReYMeN teams-pipeline token-health          # Graph token status
ReYMeN teams-pipeline token-health --force-refresh   # force a fresh token acquisition
ReYMeN teams-pipeline list                  # recent meeting jobs
ReYMeN teams-pipeline list --status failed  # only failed jobs
ReYMeN teams-pipeline show <job-id>         # full detail of one job
ReYMeN teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
ReYMeN teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
ReYMeN teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
ReYMeN teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
ReYMeN teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

ReYMeN teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
ReYMeN teams-pipeline delete-subscription <sub-id>
ReYMeN teams-pipeline maintain-subscriptions            # renew near-expiry ones
ReYMeN teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `ReYMeN teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `ReYMeN cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
