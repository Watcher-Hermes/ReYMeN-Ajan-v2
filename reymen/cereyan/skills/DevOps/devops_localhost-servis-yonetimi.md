...
description: Manage localhost services, ports, health checks, and desktop shortcuts
...
|------|--------|-------|----------|
| 9119 | ReYMeN Agent Dashboard | `ReYMeN dashboard --no-open` (background, önce build gerekebilir) |
...
NOT: Port 8648'de çalışan ayrı bir "ReYMeN Studio" yoktur. ReYMeN'in web arayüzü `ReYMeN dashboard --port 9119` komutuyla başlatılır.
...
5. **ReYMeN Dashboard build hatası** — `ReYMeN dashboard` ilk çalıştırmada `Cannot find module 'typescript/bin/tsc'` hatası verirse, şunu çalıştır:
   ```bash
   cd /c/Users/marko/AppData/Local/hermes/hermes-agent
   npm install --workspace web
   ```
   Sonra `ReYMeN dashboard --no-open` tekrar dene. Build ~45 saniye sürer.
6. **`ReYMeN dashboard` arka planda** — `terminal(background=true, notify_on_complete=true)` ile başlat. Build tamamlanınca port 9119 kontrol et.