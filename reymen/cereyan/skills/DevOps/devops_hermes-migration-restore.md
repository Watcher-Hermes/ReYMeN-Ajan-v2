---
name: ReYMeN-migration-restore
title: ReYMeN Migration Restore
description: ReYMeN Agent'i yeni bir bilgisayara tek GitHub linkiyle full restore
  etme. Skills, state.db (hafıza+session), config template ve restore script'ini tek
  repoda toplar. Güncelleme için -Update parametresiyle tekrar çalıştırılır.
tags:
- automation
- devops
- system
...
ReYMeN Agent'in **tüm yapılandırmasını, skill'lerini, hafızasını ve konuşma geçmişini** tek bir GitHub reposu (`ReYMeN-full-backup`) + tek PowerShell scriptiyle yeni bilgisayara taşımak. Tek link: `github.com/latent-ReYMeN/ReYMeN-full-backup`. Güncellemelerde aynı script `-Update` parametresiyle çalışır.
...
├── ReYMeN-full-restore.ps1          # Ana restore + update scripti
├── ReYMeN-config-template.yaml      # Config şablonu (API anahtarları maskeli)
├── ReYMeN-state-part001.zip         # state.db parça 1 (55MB)
├── ReYMeN-state-part002.zip         # state.db parça 2 (50MB)
...
```powershell
# 1. ReYMeN Agent'i kur (https://ReYMeN-agent.nousresearch.com/docs)
# 2. Repoyu klonla
git clone https://github.com/latent-ReYMeN/ReYMeN-full-backup.git
cd ReYMeN-full-backup

# 3. Restore scriptini çalıştır
powershell -ExecutionPolicy Bypass -File .\ReYMeN-full-restore.ps1
...
```powershell
powershell -ExecutionPolicy Bypass -File %USERPROFILE%\ReYMeN-backup\ReYMeN-full-restore.ps1 -Update
```
...
1. **Repo klonlama**: `git clone` → `%USERPROFILE%\ReYMeN-backup`
...
for i in range(0, len(data), chunk_size):
    part = i // chunk_size + 1
    with open(f'ReYMeN-state-part{part:03d}.zip', 'wb') as pf:
...
1. **state.db kilitli olabilir** — ReYMeN çalışırken state.db kilitlenir. Zip almadan önce `Stop-Process -Name ReYMeN` ile durdur.
